...
class TobiiGlassesController():
    ...
    def __init__(self, address = None, video_scene = True, grab_data=False, timeout = None):
        ...
        if self.address is None:
            ...
            if address is None:
                logging.debug("Using hardcoded IPv4 address")
                self.address = "192.168.0.102"
    ...
    def __refresh_data__(self, jsondata):
        ...
        try:
            ...
            sig = jsondata['sig']
            ts = jsondata['ts']
            if self.data['sig']['ts'] < 0 and (jsondata['s'] == 0):
                self.data['sig'] = jsondata
        except:
            pass
        try:
            ac = jsondata['ac']
            ts = jsondata['ts']
            if self.data['ac']['ts'] < 0 and (jsondata['s'] == 0):
                self.data['ac'] = jsondata
        except:
            pass
        try:
            gy = jsondata['gy']
            ts = jsondata['ts']
            if self.data['gy']['ts'] < 0 and (jsondata['s'] == 0):
                self.data['gy'] = jsondata
        except:
            pass
